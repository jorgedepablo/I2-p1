...
procedure Words is
  ...
  begin
    while not Eol loop -- Eol ahora mismo esta a false
      Posicion := ASU.Index(Line, " "); --Busca dentro de Line un " " espacio en blanco y te da un numero natural de donde lo encuentra
      if ASU.Length(Line) = 0 then --Esto te da si la longitud de linea es 0
        ...
        if Posicion = 0 then -- Posibilidad de que no tenga espacios en blanco
          Word_Lists.Add_Word(List, Line);-- Mete la palabra a la lista directamente
  ...
  begin
    ...
    ATI.Put_Line("|" & ASU.To_String(Word)& "| Deleted"); --Evitamos que escriba words_list
  ...
  Procedure Search_Word (List: in out Word_Lists.Word_List_Type; Word: in ASU.Unbounded_String) is
      C: Natural := 0;
  begin
    Word_Lists.Search_Word(List, Word, C); -- Hacemos que nos pase le count en out (definido en el paquete)
    ATI.Put_Line("|" & ASU.To_String(Word)& "|" & Integer'Image(C)); --Evitamos que Word_Lists escriba cosas

  exception
    when Constraint_Error => -- Este error salta si no encuentra la palabra
      ATI.Put_Line("|" & ASU.To_String(Word) & "| 0"); -- Pone esto

  end Search_Word;

  procedure Most_Frecuent_Word (List: in Word_Lists.Word_List_Type) is
