...
begin
--ESTE PROGRAMA ES SOLO PARA PROBAR EL CAMBIO DEL EXAMEN DE CONTAR LAS PALABRAS SOLAS, EL BUENO ES EL WORDS_1.0 !!!!!
  if ACL.Argument_Count = 1  then
    File := ASU.To_Unbounded_String(ACL.Argument(1));
    ATI.Open(Fich, ATI.In_File, ASU.To_String(File));
    Cut_Text(Fich, List);
    Most_Frecuent_Word(List);
    ATI.Put_Line(Integer'Image(Word_Lists.Single_Words(List)));
